...
**解释**
```
engine=myisam不支持事务处理等高级处理，engine=innoDB支持
MyISAM类型的表强调的是性能，其执行速度比InnoDB更快，但是不提供事务支持，而InnoDB提供事务支持以及外部键等高级数据库功能
这样就可以根据数据表不同的用处来使用不同的存储类型
```
...
告诉服务器发送的数据是什么类型的。character_set_client
告诉转换器,转换什么编码character_set_connection
查询结果用什么编码character_set_results
...
***
# 连接数据库
***demo1.py(https://github.com/Harrdy2018/MySQL/blob/master/demo1.py)***
